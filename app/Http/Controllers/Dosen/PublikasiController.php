...
class PublikasiController extends Controller
{
    public function index()
    {
        ...
        return view('pages.dosen.publikasi.index')
            ->with('publikasi',$publikasi);
    }
    public function detail($id)
    {
        $publikasi=Publikasi::where('id',$id)->with('mahasiswa')->first();
        return view('pages.dosen.publikasi.detail')->with('publikasi',$publikasi);
    }
    public function form($id=-1)
    {
        ...
        return view('pages.dosen.publikasi.form')
            ->with('det',$det)
            ->with('id',$id);
    }
    ...
    public function simpan(Request $request,$id=-1)
    {
        ...
        if($id==-1)
            return redirect('dosen-publikasi-ilmiah')->with('status','Data Publikasi Berhasil Ditambah, Akan Segera Di Verifikasi Oleh Pimpinan Departemen');
        else
            return redirect('dosen-publikasi-ilmiah')->with('status','Data Publikasi Berhasil Di Edit');
    }
    ...
    public function verifikasi($id,$status,$pimpinan)
    {
        ...
        if($status==1)
        {
            if($pimpinan=='dep')
            {
                ...
                $notif->to=$user->id;
                $notif->flag_active=1;
                $notif->pesan="Mahasiswa : <b>".$pub->mahasiswa->nama."</b> Mengajukan Publikasi Ilmiah Untuk Di Verifikasi<br><a href='".url('dosen-publikasi-ilmiah')."'>Klik Disini</a>";
                $notif->save();
                return redirect('dosen-publikasi-ilmiah')->with('status','Data Publikasi Berhasil Di Verifikasi, Dan Akan Di Verifikasi Manajer Akademik');
            }
            elseif($pimpinan=='mandik'){
                ...
                $notif->save();
                return redirect('dosen-publikasi-ilmiah')->with('status','Data Publikasi Berhasil Di Verifikasi');
            }     
        }
        else
            return redirect('dosen-publikasi-ilmiah')->with('status','Hapus Verifkasi Data Publikasi Berhasil');
    }
    public function destroy($id)
    {
        Publikasi::find($id)->delete();
        return redirect('dosen-publikasi-ilmiah')->with('status','Data Publikasi Berhasil Dihapus');
    }
}
