...
@section('konten')
        ...
        <!-- END PAGE BAR -->
        <!-- BEGIN PAGE TITLE-->
        <h1 class="page-title"> Quota Penguji
            <small>Data</small>
        </h1>
        <!-- END PAGE TITLE-->
        <!-- END PAGE HEADER-->
...
<script>
    ...
    {
        $('#loader').show();
        $('#data').load('{{url("quota-penguji-data")}}',function(){
            $('#sample_4').dataTable();
            $('#loader').hide();
        });
    }
                ...
                bootbox.confirm({
                    ...
                    callback: function (result) 
                    {
                        if(result)
                        {
                            if(id==-1)
                            {
                                var t_url = '{{url("quota-penguji")}}';
                            }
                            else
                                var t_url = '{{url("quota-penguji")}}/'+id;
                            ...
                            else
                            {
                                $.ajax({
                                    ...
                                    if(id==-1)
                                    {
                                        var ps="Quota Penguji Berhasil Disimpan";
                                    }
                                    else
                                    {
                                        var ps="Quota Penguji Berhasil Di Edit";
                                    }
                                    swal("Berhasil", ps, "success");
                                }).fail(function(dt){ 
                                    var ps='Quota Penguji Gagal Disimpan';
                                    pesan(ps,'error');
                                });
                            }
    ...
    function hapus(id)
    {
        ...
        function(isConfirm) {
            if (isConfirm) {
                $.ajax({
                    url : '{{url("quota-penguji-hapus")}}/'+id,
                    dataType : 'JSON'
                }).done(function(){
                    loaddata();
