        ...
        <div class="">
            <div class="portlet light portlet-fit portlet-datatable bordered">
                <div class="portlet-body">
                    <!-- BEGIN FORM-->
                    <form action="{{$id==-1 ? url('staf-admin') : url('staf-admin/'.$id) }}" class="horizontal-form" id="form-Staf" method="POST">
                        {{ csrf_field() }}
                        @if ($id!=-1)
                            {{ method_field('PATCH') }}
                        @endif
...
<script>
    $(document).ready(function(){
        ...
        $('#simpan').on('click',function(){
            ...
            {
                pesan("Nama Staf Harus Diisi",'error');
                $('#nama').focus();
            }    
            else if(departemen=='')
            {
