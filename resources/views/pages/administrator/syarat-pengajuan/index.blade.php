...
@section('title')
    <title>Data Syarat Pengajuan :: SIMASKUS</title>
@endsection
...
<script>
    ...
    {
        $('#loader').show();
        $('#data').load('{{url("master-syaratpengajuan-data")}}',function(){
            $('#sample_4').dataTable();
            $('#loader').hide();
        });
    }
    ...
    {
        if(val==0)
            location.href='{{url("master-jenispengajuan")}}';
    }
    function loadform(id)
    {
        $.ajax({
            url : '{{url("master-syaratpengajuan")}}/'+id,
            success: function(html){
                bootbox.confirm({
                    title: "Form Syarat Pengajuan",
                    message: html,
                    buttons: {
                        cancel: {
                    ...
                    callback: function (result) 
                    {
                        if(result)
                        {
                            if(id==-1)
                            {
                                var t_url = '{{url("master-syaratpengajuan")}}';
                            }
                            else
                                var t_url = '{{url("master-syaratpengajuan")}}/'+id;
                            ...
                            else
                            {
                                $.ajax({
                                    ...
                                    if(id==-1)
                                    {
                                        var ps="Data Syarat Pengajuan Berhasil Disimpan";
                                    }
                                    else
                                    {
                                        var ps="Data Syarat Pengajuan Berhasil Di Edit";
                                    }
                                    swal("Berhasil", ps, "success");
                                }).fail(function(dt){ 
                                    var ps='Data Syarat Pengajuan Gagal Disimpan';
                                    pesan(ps,'error');
                                });
                            }
    ...
    function hapus(id)
    {
        ...
        function(isConfirm) {
            if (isConfirm) {
                $.ajax({
                    url : '{{url("master-syaratpengajuan-hapus")}}/'+id,
                    dataType : 'JSON'
                }).done(function(){
                    loaddata();
